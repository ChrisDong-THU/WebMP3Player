$(function(){
    ...
    const orderh = urlParams.get('order');

    // 如果order参数存在，使用它初始化播放器
    if (orderh) {
        initPlayer(parseInt(orderh)); // 确保转换为数字类型，如果 'order' 应该是数字
    } else {
        initPlayer(); // 没有提供order参数时，调用无参数的initPlayer
    }
});